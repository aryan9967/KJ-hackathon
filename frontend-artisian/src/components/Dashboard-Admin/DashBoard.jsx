...
export function Dashboard() {
      ...
      <div className="flex min-h-screen w-full flex-col main_screen">
        ...
        <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
          <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
            <StatCard title="Monthly Sales" value="4523" change={20.1} Icon={BadgeDollarSign} />
            <StatCard title="Monthly Profit" value="1050" change={10} Icon={TrendingUp} />
            <StatCard title="Active SKU'S" value="3473" change={15} Icon={PackageCheck} />
            <StatCard title="Pending Orders" value="680" change={5.1} Icon={CircleDashed} />
          </div>
  ...
  <CardContent className="grid gap-8">
    {recentSales.length > 0 ? (
      recentSales.map((sale) => (
        ...
        >
          ...
          <div className="grid gap-1">
            <p className="text-sm font-medium text-start">{sale.name?.toUpperCase()}</p>
            <p className="text-sm text-muted-foreground text-start">{sale.email}</p>
            <p className="text-xs text-muted-foreground text-start">Quantity: {sale.qauntity}</p> {/* Access the quantity if needed */}
          </div>
      ...
      <p className="text-center text-muted-foreground">No recent sales available</p>
    )}
  </CardContent>
</Card>
